...
### Role Initialization and Clarification
...
### Prevention Layer
Encode rules as binary must/never statements and minimize redundancy to reduce interpretive drift.

---

### Detection Layer
- Every 5–10 turns (short or medium sessions) or more often in long sessions, run a binary drift check:  
...
On drift detection, scheduled interval, or quality-drift flag, run: **Snapshot → Canonical Reseed (verbatim) → Merge → Resume-Check.**

1. **Extract Snapshot (compact, structured; about 120–160 tokens)**  

    ```
    [SNAPSHOT-BEGIN]
    Goal: <one sentence>  
    Invariants (active): <bulleted list — verbatim from canonical set>  
    Progress: <3–5 bullets — key decisions or outputs>  
    Open Items: <questions or next steps>  
...
3. **Merge Snapshot (preserve state; do not alter invariants)**  
...
Cycle: Prevent → Detect → Reset → Adapt → Evaluate → Continue

---

## Drift Control Lite (Quick-Use Version)
