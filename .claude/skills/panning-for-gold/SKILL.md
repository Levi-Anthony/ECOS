...
description: Use when processing voice transcripts, brain dumps, stream-of-consciousness notes, or any raw multi-topic capture. Extracts every idea thread, then evaluates each one with deep brainstorming, then captures results to BRAIN (ECB). Trigger on transcripts, exports, "process this", "pan for gold", "brain dump", "what did I say", or multi-topic markdown files.
author: Jared Irish
version: 2.0.0
---
...
3. **EVALUATORS WRITE TO FILES.** Every background evaluator agent MUST write its evaluation to a permanent file (e.g., `Operations/transcripts/evaluations/YYYY-MM-DD-{slug}.md`) as part of its task. Do not depend on collecting agent return values.
...
File naming: `Operations/transcripts/YYYY-MM-DD-{source}-transcript.md` or `Operations/transcripts/brainstorming/YYYY-MM-DD-{topic}.md`
...
**IMMEDIATELY** save the Phase 1 inventory to `Operations/transcripts/YYYY-MM-DD-{source}-inventory.md` or equivalent. This file survives compaction even if nothing else does.
...
5. **Connections:** How does this connect to their existing thinking? (Use search_thoughts to find related BRAIN entries.)
6. **Verdict:** One of:
...
- Output path: `Operations/transcripts/evaluations/YYYY-MM-DD-{idea-slug}.md`
...
`Operations/transcripts/YYYY-MM-DD-{source}-gold-found.md`
...
## Phase 3.5: Capture to BRAIN (ECB)

After writing the gold-found file, capture the harvest to BRAIN. Per the ECOS capture-gate — the single canonical statement lives in the Operational Kernel; **defer to it, do not restate it** — routine evidence-grade captures of life/work ideas are *act-freely*: capture them directly, no permission theater. Two gates still bind:

- **BRAIN-scope gate.** BRAIN holds memory in service of Levi's life and work in the world (tango/ECTango, TTC, IT consulting, the people he works with, personal knowledge). It does NOT hold the system's notes about itself. If a thread is about ECOS/ECB architecture, schemas, doctrine, tooling, kernels, or agent behavior, route it to the substrate (an artifact/spec/proposal) — **never** `capture_thought`.
- **Structural inner-gate.** If a thread would set a durable rule, vocabulary, protocol, taxonomy, or authority change (instruction-grade / doctrine-shaping), **propose it first** — do not auto-capture doctrine. Evidence-grade life/work ideas are not structural; capture them freely.

Before capturing, atomize per the `brain` skill (C1 one center of mass; C3 complete in isolation — defer to that skill, do not re-derive) and tag each capture with a `source` in metadata (the relevant domain tag).

1. **Each ACT NOW item** that passes the scope + inner gates gets its own `capture_thought`:
   ...
   - `metadata`: include a `source` tag.

2. **Session summary** (life/work scope only) as one `capture_thought`:
...
**No silent failures.** If a `capture_thought` call fails, surface the error explicitly — never swallow it or report success blind.

**Report what landed (legibility).** After capturing, present the capture list (entry IDs / one-line summaries) plus a one-line skip list: threads NOT captured and why (out of BRAIN scope → routed to substrate; structural → proposed; too transient/logistical). This is the same skip-list discipline the kernel requires of any capture set.

This closes the flywheel: panning extracts and evaluates, BRAIN stores the life/work signal, boot retrieval finds it next session.
